...
| 012 | Classifier scope is hardcoded to docs/+wiki/, missing everything else | Not Started | `ac-classify`'s discovery globs never look outside `docs/**` and `wiki/{entities,concepts,sources}/**` (+ index/log for bleed) — `.claude/`, `.smaqit/history/`, `.smaqit/tasks/`, `raw/` are structurally invisible. Real incident: a session-history file with capital figures/investor PII was caught only by a manual grep, not the classifier; an accidentally-tracked `.claude/projects/` (agent memory store) sat undetected since project week 1. Proposes an opt-in scope declaration (default: no change) plus a separate tool-state-directory check |
| 008 | Global classification scanner | Not Started | Global Claude Code skill (`~/.claude/skills/agentic-cms-classify/`, installs on first binary run) + optional warning-gated `agentic-cms hooks enable-global` git pre-commit hook via `core.hooksPath`, chaining into per-project `.agentic-cms/hooks/pre-commit` when present. Motivated by a real incident: this repo (never `init`-ed on itself) had zero coverage when a direct git commit landed real PII, caught only by manual review. Depends on task 005 (Completed, v0.5.0) and now inherits task 011's ack mechanism + widened floor for free via the shared ac-classify engine. NOTE: will supersede task 006's "no global agent/skill directories to seed" design decision — revisit that bullet |
...
| 011 | Classification adoption gaps (first real-world rollout) | 2026-08-19 | PR #9; delta-scoped pre-commit gate (blocks staged delta only, tree drift → one summary warning); user-only hash-bound floor ack (`ac-page classify --ack-floor`); currency floor WIDENED not narrowed (0% false-negative recall contract — a mid-task user ruling reversed the original narrowing plan), false positives resolved via ack; CONTENT.md gained a `## Classification` section + `init`/`update` schema reconciliation (report + `.agentic-cms/CONTENT.upstream.md` sidecar, never edits the user's file); `.agentic-cms/VERSION` auto-stamped from the binary on every run. New `engine_test.go`/`reconcile_test.go` — first fixture tests for the classification engine. Released as v0.7.0 |
