...
| 011 | Classification adoption gaps (first real-world rollout) | In Progress | Four gaps from v0.5.0/v0.6.x's first brownfield adoption: pre-commit gate blocks on whole tree not staged delta; no user floor-ack mechanism (false positives = permanent blocks or --no-verify); bare-currency C2 floor over-broad (~2/3 false positives); CONTENT.md schema evolution never reaches installed projects (reconciliation report + CONTENT.upstream.md sidecar). Plus VERSION stamp hygiene. Lands in shared ac-classify engine — 008 inherits |
