...
| 005 | First-class content classification (auto CIA rating) | Not Started | `classification: C0–C3` frontmatter auto-assigned by agent at write time against a CONTENT.md rubric; toolkit validation (`ac-page --classification`/`classify`, `ac-inventory` reporting); lint bleed/ratchet enforcement. Refined 2026-08-18: update-drift coverage — new `ac-classify` checker (`classified-hash` staleness + heuristic floors, raise-only) wired into cross-platform post-tool-use hooks (Claude Code/Codex/Copilot, one stdin adapter), a git pre-commit gate on staged blobs, skill verify tails, and lint sweep. 004 dependency satisfied (merged, v0.3.0). Segregation/vault out of scope — follow-up task once downstream pattern settles |
